  ...
  render() {
    ...
    return (
      <View className='search'>
        <SearchInput
          onActionClick={this.handerSearch.bind(this)}
          onCancleClick={this.handerCancel.bind(this)}
          onRef={(ref) => { this.searchInput = ref }}
        />
        {
          !showResult
            ?
            <SearchHistory onClick={(tag) => { this.searchInput.onChange && this.searchInput.onChange(tag) }} />
            :
            <View className='search-result'>
              <SearchScreen onSwitchTab={this.switchTab.bind(this)} />
              <View className='result-goods'>
                {!isReachBottom &&
                  <Loading loading={loading} hasMore={hasMore} fontSize={28} content='没有查询到相关商品～' />
                }
                ...
                {
                  isReachBottom &&
                  <Loading loading={loading} hasMore={hasMore} fontSize={28} content='我也是有底线的～' />
                }
              </View>
            </View>
        }
