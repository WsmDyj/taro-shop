...
export default @connect(state => state.home, { onGoodsChange })
class Hot extends Taro.Component {
  static defaultProps = {
    list: []
  };
  ...
  render() {
    ...
    return <View className="hot">
        <View className="hot-title">
          <Image className="hot-title__img" src={hotPng}></Image>
          <Text>爆品推荐</Text>
        </View>
        <View className="hot-wrap">
          <Swiper className="hot-swiper" circular autoplay interval={3000} current={this.state.active} onChange={this.swiperChange.bind(this)}
        // indicatorDots
        // indicatorActiveColor='#e1251b'
        >
            {list.map((group, index) => <SwiperItem key={index} className="hot-swiper__item">
                {group.map(item => <View key={item.id} className="hot-item" onClick={this.handleClick.bind(this, item)}>
                ...
                {group.length === 2 && <View className="hot-item__fill"></View>}
              </SwiperItem>)}
          </Swiper>
          <View className="hot-swiper__dots">
            {list.map((item, index) => {
            return <View className={index === this.state.active ? "hot-swiper__active" : 'hot-swiper__dot'} key={index}>
                  </View>;
          })}
          </View>
        </View>
