...
export default class imagerLoader extends Taro.Component {
  ...
  render() {
    ...
    return <Nerv.Fragment>
        {imgUrl && <Image lazy-load className={this.props.placeholder} src={imgUrl} />}
        <Nerv.Fragment>
          {imgLoadList.map((item, index) => <Image key={index} src={item} className={this.props.placeholder} data-src={item} binderror={this.binderror} onLoad={this.imgLoader._imgOnLoad.bind(this.imgLoader)} onError={this.imgLoader._imgOnLoadError.bind(this.imgLoader)} style="width: 0; height: 0; opacity: 0" />)}
        </Nerv.Fragment>
      </Nerv.Fragment>;
  }
}